...
foreach my $vinf (keys%variants){
		...
		if($variants{$vinf}{vclass} eq "snv"){
				my $fa19 =&faidx($variants{$vinf}{bed_region},$hg19);
				if($vinf !~ /:$fa19$/){die "ERROR:SNV position error?? $vinf\n";}
...
open(LER,"|gzip -c >$input"."_liftovererror_inGRCH38.maf.gz");
if($input_maf =~ /.gz$/){
		open(IN,"gunzip -c $input_maf|") or die "ERROR::cannot gunzip open $input_maf\n";
}else{
		open(IN,"$input_maf") or die "ERROR::cannot open $input_maf\n";
...
print OUT "$header\n";
print LER "$header\tref_check\tref_convert\n";
while(<IN>){
		chomp;
		my @line = split(/\t/,);
		my($chr,$posi,$ref) = ($line[$col{chr}],$line[$col{posi}],$line[$col{ref}]);
		my $vinf = "$chr:$posi:$ref";
		if(!defined $remap{$vinf}){print LER "$_\tliftover_error\tliftovrer_error\n";next;}
		my($chr38,$start38,$end38);
		if($remap{$vinf} =~ /^(.+):(\d+)-(\d+)$/){($chr38,$start38,$end38)=($1,$2,$3);}else{print "lifover error?? $vinf => $remap{$vinf}\n";}
		if($chr ne $chr38){print LER "$_\tchromosome_change\t$vinf>$chr38:$start38-$end38\n";next;}
		if($variants{$vinf}{vclass} eq "snv"){
				$line[$col{posi}]=$end38;
				if(!defined $variants{$vinf}{refcheck}){
						print OUT join("\t",@line)."\n";
				}else{
						print LER "$_\tref_changed\t$variants{$vinf}{refcheck}\n";
				}
		}elsif($variants{$vinf}{vclass} eq "ins"){
				$line[$col{posi}]=$start38+1;
				if(!defined $variants{$vinf}{refcheck}){
						print OUT join("\t",@line)."\n";
				}else{
						print LER "$_\tref_changed\t$variants{$vinf}{refcheck}\n";
				}
		}elsif($variants{$vinf}{vclass} eq "del"){
				$line[$col{posi}]=$start38+2;
				if(!defined $variants{$vinf}{refcheck}){
						print OUT join("\t",@line)."\n";
				}else{
						print LER "$_\tref_changed\t$variants{$vinf}{refcheck}\n";
				}
		}else{die "ERROR::what vaiant $vinf; $_\n";}
}
close IN;
close OUT;
close LER;

...
if(0){
  ...
  write_df(duplicate_site_cont,"/Volumes/areca42TB2/gdc/control_region/all_patient/duplicate_site_control_gnomad.tsv")
}
duplicate_site_cont = 
  read_tsv("/Volumes/areca42TB2/gdc/control_region/all_patient/duplicate_site_control_gnomad.tsv") 
...
if(0){
  maf_trim_for_cumulative_cont = function(.maf=norm_maf_all_cont,.vcf=control_gnomad,.race="all",.fdr=0.01,
    ...
    .maf %>>%
      ...
      mutate(MAC = ifelse(n_allele1==alt,2,ifelse(n_allele2==alt,1,0)),age = age) %>>%
      dplyr::select(patient_id,cancer_type,gene_symbol,chr,start,end,ref,alt,mutype,AF,MAF,MAC,age,Protein_position,FILTER) %>>%
      #rbind(ref_minor) %>>%
      filter(!is.na(age))
  }

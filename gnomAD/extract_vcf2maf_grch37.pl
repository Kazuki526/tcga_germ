...
my @ls = `ls file|grep $gnomAD_version`;
...
sub print_extracted_file( $ ){
		my $chr = $_[0];
		my $infile = "file/$gnomAD_version.$chr.vcf.bgz";
		open(VCF,"gunzip -c $infile|");
		open(ALL,"|gzip -c >maf/all_maf/gnomAD_chr$chr.maf.gz");
		open(CAN,"|gzip -c >maf/non_cancer_maf/non_cancer_chr$chr.maf.gz");
		open(CONT,"|gzip -c >maf/control_maf/control_chr$chr.maf.gz");

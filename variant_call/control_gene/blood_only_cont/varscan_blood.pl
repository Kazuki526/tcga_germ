...
if(!-e "$contdir/blood"){mkdir "$contdir/blood";}
...
#debug
		...
		if(!-e "$contdir/blood/$CT"){mkdir "$contdir/blood/$CT";}
		if(!-e "$contdir/blood/$CT/maf"){mkdir "$contdir/blood/$CT/maf";}
		if(!-e "$contdir/blood/$CT/out"){mkdir "$contdir/blood/$CT/out";}
		if(!-e "$contdir/blood/$CT/vcf"){mkdir "$contdir/blood/$CT/vcf";}
		if(!-e "$contdir/blood/$CT/depth"){mkdir "$contdir/blood/$CT/depth";}
		my $vcf_snp ="$contdir/blood/$CT/out/$pid.snp.vcf";
		my $maf ="$contdir/blood/$CT/maf/$pid.maf";
		if($nsolid == 0){
				if(!-e $maf){`cp $contdir/$CT/maf/$pid.maf $maf`;}
		}else{
#merge multi bam files
				my $norm_bam="";
				if($nnorm>1){
						for(my $i=0;$i<$nnorm;$i++){$norm_file[$i]="$contdir/$CT/norm_bam/$norm_file[$i]";}
						$norm_bam = "$contdir/$CT/norm_bam/$pid"."_blood.bam";
...
#varscan
				if(!-e $vcf_snp){
						...
						`zsh -c \"varscan somatic <\($mpile\) $contdir/blood/$CT/out/$pid --tumor-purity $purity --p-value 0.1 --output-vcf 1 --mpileup 1\"`;
				}
				if(!-e $maf){
						print "$line:start $CT:$pid vcf2maf\n";
						`zsh $ENV{HOME}/git/tcga_germ/variant_call/vcf2maf.sh $pid $contdir/blood/$CT 2>&1`;
				}
		}

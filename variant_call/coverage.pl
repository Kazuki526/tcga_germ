...
$line=0;
#make coverage file
open(ARM,"|gzip -c >$tdgdir/all_patient/ref_minor_coverage.tsv.gz");
print ARM "patient_id\tchr\tstart\tfocal\n";
open(APCOV,">$tdgdir/all_patient/patient_coverage.tsv");
print APCOV "patient_id\trole\tcoverage\n";
open(APDEP,">$tdgdir/all_patient/patient_depth.tsv");
...
foreach my $CT (sort keys %bam_list_fordepth){
		...
		if((-e $patient_cov_file)&&(-e $site_cov_file)&&(-e $ref_minor_file)&&(-e $patient_dep_file)){
...
#calculate coverage and depth
		...
		for(my $group=1;$group-1 < $num/50;$group++){
				...
				while(my $nline = <NDP>){
						my $tline = <TDP>;
						my @nline = split(/\t/,$nline);if($nline[0] eq "chrX"){next;}
						...
						my $role = $posi2role{$nline[0]}{$nline[1]};
						for(my $i =2;$i<scalar(@nline);$i++){
								if($patient_inf{$dp_col[$i]}{age} eq "NA"){next;}
								$patient_depth{$dp_col[$i]}{$role}{norm}+=$nline[$i];
								$patient_depth{$dp_col[$i]}{$role}{tumor}+=$tline[$i];
								$patient_depth{$dp_col[$i]}{$role}{bp}++;
								if(($nline[$i] >=8) && ($tline[$i] >=8)){
										...
										$patient_coverage{$dp_col[$i]}{$role}++;
		...
		foreach my $pid(sort keys %patient_coverage){
				print PCOV  "$pid\tTSG\t$patient_coverage{$pid}{TSG}\n";
				print PCOV  "$pid\toncogene\t$patient_coverage{$pid}{oncogene}\n";
				print PCOV  "$pid\toncogene/TSG\t".$patient_coverage{$pid}{'oncogene/TSG'}."\n";
				print APCOV "$pid\tTSG\t$patient_coverage{$pid}{TSG}\n";
				print APCOV "$pid\toncogene\t$patient_coverage{$pid}{oncogene}\n";
				print APCOV "$pid\toncogene/TSG\t".$patient_coverage{$pid}{'oncogene/TSG'}."\n";
		}
		close PCOV;
		open(PDEP,">$patient_cov_file");
		print PDEP "patient_id\trole\tnorm_depth\ttumor_depth\tlength_bp\n";
		foreach my $pid(sort keys %patient_depth){
				print PDEP  "$pid\tTSG\t$patient_depth{$pid}{TSG}{norm}\t$patient_depth{$pid}{TSG}{tumor}\t$patient_depth{$pid}{TSG}{bp}\n";
				print PDEP  "$pid\toncogene\t$patient_depth{$pid}{oncogene}{norm}\t$patient_depth{$pid}{oncogene}{tumor}\t$patient_depth{$pid}{oncogene}{bp}\n";
				print PDEP  "$pid\toncogene/TSG\t".$patient_depth{$pid}{'oncogene/TSG'}{norm}."\t";
				print PDEP  $patient_depth{$pid}{'oncogene/TSG'}{tumor}."\t".$patient_depth{$pid}{'oncogene/TSG'}{bp}."\n";
				print APDEP "$pid\tTSG\t$patient_depth{$pid}{TSG}{norm}\t$patient_depth{$pid}{TSG}{tumor}\t$patient_depth{$pid}{TSG}{bp}\n";
				print APDEP "$pid\toncogene\t$patient_depth{$pid}{oncogene}{norm}\t$patient_depth{$pid}{oncogene}{tumor}\t$patient_depth{$pid}{oncogene}{bp}\n";
				print APDEP "$pid\toncogene/TSG\t".$patient_depth{$pid}{'oncogene/TSG'}{norm}."\t";
				print APDEP $patient_depth{$pid}{'oncogene/TSG'}{tumor}."\t".$patient_depth{$pid}{'oncogene/TSG'}{bp}."\n";
		}
		close PDEP;
...
open(ASCOV,">$tdgdir/all_patient/site_coverage_all.tsv");
